<?php
  ...
  $draftNews = getDraftNews($conn);
  $numOfNews = getCountOfDraftNews($conn);
  ...
  if (isset($_GET['p'])) {
    ...
    $sql = "SELECT * FROM post WHERE draft = 1 ORDER BY time DESC LIMIT ".$from.",3";
  ...
  <section class="my-5">
    ...
    <?php for ($i = 0; $i <$numOfLinksPagination; $i++) : ?>
        <li class="page-item <?= (isset($_GET['p']) && $_GET['p'] == $i)? 'active' : ((!isset($_GET['p']) && ($i === 0)))? 'active' : ''  ?>">
          <a class="page-link" href="index.php?page=draft&p=<?= $i ?>"><?= $i+1 ?></a>
        </li>
    <?php endfor;?>
          <li class="page-item">
